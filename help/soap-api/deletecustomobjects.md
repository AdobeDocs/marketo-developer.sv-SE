...
description: Lär dig använda deleteCustomObjects i Marketo SOAP API för att ta bort anpassade objekt med nyckelattribut med statusarna DELETED, UNCHANGED, FAILED plus XML, PHP, Java.
...
Tar bort ett eller flera anpassade objekt och returnerar resultatet av åtgärden (DELETED, UNCHANGED, FAILED).
...
| customObjKeyLists->keyList->attribute | Obligatoriskt | Attributet är ett nyckel/värde-par som används för att identifiera de anpassade objekt som du vill ta bort. Du kan ange flera attribut i customObjKeyLists |

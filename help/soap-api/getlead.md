...
title: getLead
feature: SOAP
description: Använd Marketo SOAP getLead för att hämta en enstaka lead och alla fältvärden via IDNUM, e-post, cookie eller SFDC-ID:n, med XML för begäran och svar plus PHP-exempel.
...
Den här funktionen hämtar en enstaka lead-post från Marketo, med alla fältvärden för inbyggda och anpassade fält, för en lead som identifieras med den angivna nyckeln (LeadKey). Om leadet finns baserat på indataparametrarna returneras leadpostattributen i resultatet.

Obs! Leadattribut som är av strängdatatyp och tomma returneras inte som en del av svaret.
